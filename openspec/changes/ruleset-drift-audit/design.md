...
- **Normalize with `jq`.** Remove top-level server metadata (`id`, `node_id`, timestamps, `_links`, and `current_user_can_bypass`) while preserving policy-bearing identifiers such as nested `actor_id` and `integration_id`; sort rules and required-check contexts by stable keys before comparing. Textual JSON formatting is not significant. Hand-written normalization was rejected as more error-prone and less readable.
