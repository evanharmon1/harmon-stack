...
printf '[{"id":42,"name":"Protect Main"}]\n' >"$list"
cp "$file" "$live"
...
if RULESET_AUDIT_FILE="$file" RULESET_AUDIT_REPO=example/ruleset RULESET_AUDIT_ERROR=permission-denied "$audit" >/dev/null 2>&1; then
