...
- `devcontainer-build.yml` — prebuilds the devcontainer images to GHCR and
  ...
  **CI-only** check, deliberately absent from the local `task ci` mirror:
  ...
  skipped at the fork trust boundary), and `devcontainer-assert-bot` stands
  down entirely on `merge_group` because its registry cache cannot go
  credential-free per event — see
